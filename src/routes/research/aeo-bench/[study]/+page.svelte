...
// AEO Bench series, with the registration + report as citations.
const studyLd = $derived.by(() => {
	const url = `https://www.lightningjar.com/research/aeo-bench/${data.study.slug}`;
	return jsonLdScript({
		"@context": "https://schema.org",
		"@type": "ScholarlyArticle",
		headline: `Study ${data.study.letters}: ${data.study.title}`,
		about:
			"Pre-registered benchmark study of whether agent-readiness and answer-engine-optimization techniques help AI agents use websites",
		datePublished: data.study.published || undefined,
		isPartOf: {
			"@type": "CreativeWorkSeries",
			name: "AEO Bench",
			url: "https://www.lightningjar.com/research/aeo-bench",
		},
		author: LJ_PUBLISHER,
		...
		citation: [
			...(data.study.brief
				? [
						`https://github.com/kevinpeckham/aeo-bench/blob/main/${data.study.brief}`,
					]
				: []),
			"https://github.com/kevinpeckham/aeo-bench/blob/main/REPORT.md",
		],
...
// aeo-studies.json currently has no related entries, so its empty array
// infers as never[]; name the shape related entries take when present.
const related = $derived(
	data.study.related as { href: string; title: string }[],
);
...
// study, for headings like "Chart 1.1: …" / "Table 1.1: …".
...
>
	<div class="max-w-article blog-article">
		...
		>
			{#if data.previous}
				<a
					href="/research/aeo-bench/{data.previous.slug}"
					class="underline decoration-maximumYellow/40 hover:decoration-maximumYellow underline-offset-4 hover:text-maximumYellow"
				>
			...
			{#if data.next}
				<a
					href="/research/aeo-bench/{data.next.slug}"
					class="underline decoration-maximumYellow/40 hover:decoration-maximumYellow underline-offset-4 hover:text-maximumYellow"
				>
			...
			{#if related?.length}
				...
				>
					...
					<ul class="grid grid-cols-1 gap-3">
						{#each related as article (article.href)}
			...
			>
				...
				<ul class="grid grid-cols-1 gap-3 leading-snug">
					{#if data.study.brief}
						<li>
							<a
								href="https://github.com/kevinpeckham/aeo-bench/blob/main/{data.study.brief}"
								class="underline decoration-maximumYellow/40 hover:decoration-maximumYellow underline-offset-4 hover:text-maximumYellow"
							>
					...
					<li>
						<a
							href="https://github.com/kevinpeckham/aeo-bench/blob/main/REPORT.md"
							class="underline decoration-maximumYellow/40 hover:decoration-maximumYellow underline-offset-4 hover:text-maximumYellow"
						>
