...
for (const id of targets) {
	...
	{
		id: "escape-hatch",
		eyebrow: "10 · The seatbelt · Studies AC, AE, AI, AG",
		title: "Give the model permission to ask",
		prose: [
			`By default a model missing context never says so: it produces a valid, plausible, silently wrong edit, hundreds of times across our studies, zero clarifying questions, not because it cannot see the gap but because nothing said asking was allowed. One sentence of permission fixes that, and it is now measured across the whole territory. On provably-unsolvable tasks, every model asked, 270 of 270 against 0 of 270 without, naming the exact missing node (an ask_user tool performed identically, so the sentence is the whole fix). On clear requests the tax is zero: no false asks in 90 cells, and "make it punchier" gets edited, not interrogated. Dangling references are covered on every tier ("undo that" with no context became a precise question where it used to be a silent guess 144 times out of 144). And the loop closes: every ask, answered in one plain-text reply, became a correct edit, 135 of 135, so wire the reply to a real UI path and trust the second turn.`,
			`Two boundaries survive the arc, both with measured fences. Ambiguity is a capability, not a prompt feature: when a request matches two visible nodes, only the frontier tier asks; mid tiers silently resolve it, and the obvious "ask if more than one node matches" amendment rescued sonnet, left gemini short of reliable, and bought nothing on the frontier, so build unambiguous references app-side instead of shipping that clause. And the sentence's zero tax presumes guideline 02: under skeleton views, models quote everything they need from an echo and still ask, because the target node "is not visible in the view." The hatch and the focus-ids contract are one system, not two features. Ship them together, or give the agent a view tool so fetching beats asking, and keep guidelines 02, 05, and 07 regardless: the contracts make the question unnecessary; the seatbelt makes whatever remains visible instead of silent.`,
		],
