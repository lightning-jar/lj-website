...
for (const id of targets) {
	...
	},
	{
		...
		prose: [
			...
			`The obvious amendment is also measured (Study AI): adding "if the request could match more than one node, ask" rescues sonnet completely, moves gemini most of the way but not reliably, taxes nothing, and on the frontier tier changes nothing except a slight uptick in interrogating vague wording requests. If you run a frontier model, skip the clause; if you run below it, the clause is free and helpful but is a mitigation, not a guarantee. Either way, unambiguous references built app-side remain the contract.`,
			`And know the sentence's fine print (Study AG): it covers dangling references too ("undo that" with no context became a precise question on every tier, where it used to be a silent guess 144 times out of 144), but its zero-tax record presumes guideline 02. Under skeleton views, models quote everything they need from an echo and still ask, because the target node "is not visible in the view." The hatch and the focus-ids contract are one system, not two features: ship them together, or give the agent a view tool so fetching beats asking.`,
		],
