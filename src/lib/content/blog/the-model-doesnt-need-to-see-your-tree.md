...
## Update: the view is a contract, not a compression

Study U found this post's one sharp edge. Everything above holds when the instruction carries every value the edit needs. But give the model a dependent edit ("set this block's content to the same value as that other block's") against a view that shows only the target, and it does not fail loudly: on all 90 such cells across both models it returned a valid patch with a silently invented value. No refusals, no errors, just confident fiction. The fix costs one line: include every node the request mentions in the focus ids, not just the edit target. With both nodes in view the score went 90 of 90, at about 1.7k median input tokens, which also beat the whole-tree prompt on accuracy at 25 times less input. The numbers are in [The Two Things Your Agent Can't See](/blog/the-two-things-your-agent-cant-see) and the [REPORT](https://github.com/kevinpeckham/barkup-bench).

And a tier note, July 2026: the views met the model we actually ship. Study AD re-ran both HTML view modes on claude-opus-4.8 at 300 to 1000 nodes and it swept 90 of 90, the first perfect view sweep in the series, at the same 2 to 4% of full-tree input. The bands are on the [dashboard](/research/barkup-bench#sec-opus).
