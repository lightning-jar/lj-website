...
Update, July 2026: the one inference in this post's advice got its own study, and the slogan survived it hardened (Study AF, judge-graded like this one, judges re-calibrated first). We tested whether making the model restate a document-read goal in its own words before rewriting would rescue the view arm above. It did not, and the manner of the failure is the finding: restatement compliance was perfect in every cell, and the restate arm still lost every decisive comparison, zero wins in ninety across three editors including opus. The model says the goal accurately and then orbits it anyway. Verbalization is not the active ingredient; where the goal comes from is. The same study measured restatement on top of the memo at parity under the primary judge, with the sensitivity judge leaning toward mild harm, so the ceremony adds nothing there either: the memo carries the goal by itself. Charts on the [dashboard](/research/barkup-bench#sec-restate).
