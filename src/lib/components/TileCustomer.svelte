<script lang="ts">
	...
	<div class="font-sans uppercase tracking-widest px-16 leading-normal mb-4 text-center text-white/60 text-[.825rem] h-[1rem] sm:hidden">
	...
	>
		...
		<div class="flex justify-center w-full">
			<div class="h-auto border-y-0 border-white py-4 border-opacity-60 w-72 sm:hidden">
				<img
					class="w-auto h-full"
					alt={ tile.logoAlt }
					height=32
		...
		<!-- logo position two -->
		<div class="hidden justify-center w-full bottom-0 z-10 mb-4 sm:flex bg-oxford/20">
			<div class="w-3/4">
				<img
					class="w-auto h-full"
					alt={ tile.logoAlt }
		...
		<div class="px-0 pb-8 relative">
			<div>
				<p class="text-cultured/90 font-light md:text-oxford text-center sm:hidden prose-md mb-2 line-clamp-6">{ text }</p>
				<div class="flex justify-center flex-wrap">
					{#each tile.tags as tag}
						<span class="text-maximumYellow text-sm inline-block mr-2 opacity-80 transition-opacity hover:opacity-100">{ "#" + tag }</span>
					{/each}
				</div>
			</div>
		</div>
