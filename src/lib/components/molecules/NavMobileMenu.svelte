...
<script lang="ts">
  // components
  import NavItem from "$a/NavItem.svelte";
  import IconArrowRight from "$a/IconArrowRight.svelte";
  import MobileMenuPrimary from '$m/NavMobilePrimary.svelte';
  import MobileMenuFlyout from '$m/NavMobileFlyout.svelte';
  ...
  function onKeyDown(e:KeyboardEvent) {
    ...
    // trap focus when mobilemenu is open
    if (mobileMenuOpen) {

      // variables & refs
      const activeEl: HTMLElement | null = (document.activeElement instanceof HTMLElement) ? document.activeElement : null;
      const otherKeys:boolean = (e.ctrlKey || e.altKey || e.metaKey);
      let array: HTMLElement[] = (activeMenu) ? Array.from(activeMenu.querySelectorAll('a')) : [];
      const firstItem: HTMLElement | null = (array[0]) ? array[0] : null;
      const lastItem: HTMLElement | null = (array.length > 0) ? array[array.length - 1] : null;
      const previousItem: HTMLElement | null = (activeEl && array.indexOf(activeEl) && array.indexOf(activeEl) - 1) ?  array[array.indexOf(activeEl) - 1] : null;
      const nextItem: HTMLElement | null = (activeEl && array.indexOf(activeEl) && array.indexOf(activeEl) + 1) ? array[array.indexOf(activeEl) + 1] : null;
      ...
      const focusNext = () => {
        ...
        (activeEl != firstItem && activeEl != hamburger && previousItem) ? previousItem.focus() : null;
      }
      const focusPrevious = () => {
        (activeEl == lastItem) ? hamburger.focus() : null;
        (activeEl == hamburger && firstItem) ? firstItem.focus() : null;
        (activeEl != lastItem && activeEl != hamburger && nextItem) ? nextItem.focus() : null;
      }
      const selectItem = () => {
      ...
      // key logic
      (["Tab", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Escape"].includes(e.key) ? e.preventDefault() : null);

      // mobile menu is open
      (((e.key == "Tab" && e.shiftKey) || e.key == "ArrowUp") && !otherKeys) ? focusNext() : null;
      ((e.key == "Tab" || e.key == "ArrowDown") && !e.shiftKey && !otherKeys) ? focusPrevious() : null;
      ((e.key == "Escape" || e.key == "ArrowLeft") && !e.shiftKey && !otherKeys) ? escape() : null;
  ...
  function openFlyout(flyout: Flyout) {
    ...
    const confirmedBack: HTMLElement | null = (back instanceof HTMLElement) ? back : null;
