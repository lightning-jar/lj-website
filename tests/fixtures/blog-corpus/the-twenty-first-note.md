...
## Update: the notice learned to ask for everything back (July 19)

Two studies later, the cap edge is closed from a direction the first update only hinted at. The prompt-side fence — the one clause this post warned against shipping unmeasured — was measured (Study AL) and failed its gate, on a client-prune baseline that halved between two identical temperature-zero runs three days apart. But twice during those studies, something unregistered kept happening: told by the eviction notice that its update had cost a note, the frontier model occasionally re-sent the memo compressed into fewer sentences, losing nothing. Study AM measured whether the notice itself could ask for that. One sentence appended to the eviction notice — the same facts, rules, and goals rewritten into fewer, denser notes; nothing needs to be lost — and the occasional trick became the behavior: forty of forty cap-edge evictions recovered losslessly, every one of twenty-one planted needles surviving, facts staying facts and rules staying rules through the rewrite, at the cost of one extra tool call. The tiers below the frontier ignore the invitation, harmlessly. The sentence shipped the same day, and the lesson is the same one this series has now measured three times: where you say something matters more than saying it. A rule buried in the agent's standing prompt moved nothing; the same intent, delivered by the app at the exact moment of injury, recovered everything.
