...
Update, two days later: the memo had one disclosed asterisk left. In these studies the test harness wrote it, perfectly, every time. What we shipped delegates the writing to the agent through a tool and a prompt rule, with a history window alongside, and that combination hides a specific danger: while a declaration is still inside the window, an agent that never writes the memo looks identical to one that does. Study W built sessions long enough to cross the window and audited the shipped mechanism verbatim, on three models including the production tier. The agent writes the memo faithfully (recall 36 of 36 per model, retractions handled, zero noise), keeps writing it when history makes it redundant, and the post-truncation safety net holds. The audit is in [Who Writes the Memo?](/blog/who-writes-the-memo).

One more, completing the set: Study X found the third thing the agent cannot see, the previous edit itself. "Undo that" failed 0 for 144 without a carrier, every failure a silent guess, and the measured fix is a one-line last-edit echo the application appends mechanically, no agent judgment involved. Memos carry what the user declared; the echo carries what the app just did; views carry what the document holds. The full story is in [Undo That](/blog/undo-that).

Update, three days later: the silent invention this post documents got its most surprising sequel. Study AC re-ran these exact unsolvable tasks with one sentence added to the prompt, permission to reply NEED-INFO instead of a patch, and every model asked on every cell, 270 of 270, naming the exact node it could not see, with zero false asks on the solvable twins. The invention was never blindness; nothing had told the model asking was allowed. The focus-ids contract in this post remains the fix that makes the question unnecessary; the hatch is the seatbelt that makes any residual failure visible. Details in [The Model Always Knew What It Couldn't See](/blog/the-model-always-knew).
